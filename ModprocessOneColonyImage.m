function colony = ModprocessOneColonyImage(filename, dataDir, varargin)
     ...
     elseif strcmp(type,'SIP')
         ...
         if isempty(adjustmentFactor) adjustmentFactor = 0.5; 
         end
     ...
     mask = nuclei > t;
     disp('find colonies');
     [colony, cleanmask] = findColonies(mask, [], meta, clparameters);
     disp('process individual colonies')
     b = colony.boundingBox;
     colnucmask = mask(b(3):b(4),b(1):b(2));
 %     se = strel('disk',4,6);
 %     colnucmask = imdilate(colnucmask,se);
 %     colnucmask = imfill(colnucmask,8,'holes');
 %     se = strel('disk',4,6);
 %     colnucmask = imclose(colnucmask,se);
 %     colnucmask = imfill(colnucmask,8,'holes');
 %     figure;imshow(colnucmask,[])
