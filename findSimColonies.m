function [colonies, cleanmask, welllabel] = findSimColonies(mask, range, meta, clparameters)
...
cleanmask = mask(range(3):range(4),range(1):range(2));   %when applying it to simulation images only keep this line
...
%The stats of the colonies should be always the same in the simulations:
%I am filling the structure with default values to avoid the ga to fail due
%to non existent colony - if that is the case then my cost will be bad so i
%do not care anyway
% bb =  [16.5000   16.5000   68.0000   68.0000];
stats.Eccentricity = 0;
stats.Centroid  = [50.5000   50.5000];
stats.BoundingBox = [16.5000   16.5000   68.0000   68.0000];
...
if numel(colRadiusPixel) > 1
    ...
    [~,colType] = histcounts(linsize,edges);
else
